...
class GaussianRegressionLayer(torch.nn.Module):
    ...
    def __init__(self,
        ...
        # Create a separate network for each separate univaraite Gaussian measurement that will be predicted 
        self.regression_layers = torch.nn.ModuleDict({})
        if measurement_tokens is None:
            logging.warning("No tokens are linked to univariate Normal values. Check this is intended behaviour")
        else:
