...
def run_job(pre_trained_model, seed, sweep, record_metrics=None):
    ...
    with initialize(version_base=None, config_path="../../../confs", job_name="testing_notebook"):
        cfg = compose(config_name="config_CompetingRisk11M", 
                      overrides=[# Experiment setup
                                 ...
                                 "experiment.project_name='SurvivEHR-CVD-fine-tuning'",
                                 f"experiment.run_id='{pre_trained_model}'",
                                 f"experiment.fine_tune_id='CVDAblation/ce9091d_4LN_{sweep_name}_{seed}'",
    ...
    cfg.fine_tuning.fine_tune_outcomes=[
        "IHDINCLUDINGMI_OPTIMALV2", "ISCHAEMICSTROKE_V2", "MINFARCTION", "STROKEUNSPECIFIED_V2", "STROKE_HAEMRGIC"

    ]

    # Backbone LR logic
    if cfg.data.subsample_training is not None:
        if cfg.data.subsample_training < 10000:
            cfg.optim.learning_rate = 1e-4
        elif cfg.data.subsample_training < 100000:
            cfg.optim.learning_rate = 5e-5
        else:
            cfg.optim.learning_rate = 5e-5
    ...
    # # Logic for adjusting based on sample size
    # if [ "$SAMPLE_SIZE" -lt 10000 ]; then
    #     backbone_lr=1e-4
    # elif [ "$SAMPLE_SIZE" -lt 100000 ]; then
    #     backbone_lr=5e-5
    # else
    #     backbone_lr=5e-5
    # fi
    ...
    if wandb.run:
        ...
        results_train = record_results(summary, "CVD-ablation", pre_trained_model, seed, record_metrics)
    wandb.finish()

    # Save results
    save_path = "examples/modelling/SurvivEHR/notebooks/CompetingRisk/2_1_Cardiovascular_disease/"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sweep", type=str, default="experiment.project_name='SurvivEHR-CVD-fine-tuning'")
