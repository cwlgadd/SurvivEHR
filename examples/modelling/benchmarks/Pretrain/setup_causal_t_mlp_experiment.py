...
class CausalTMLPExperiment(pl.LightningModule):
    def __init__(
        ...
        dims = [cfg.transformer.n_embd] + hidden_dims + [vocab_size]
        layers = []
    ...
    def forward(self, batch, is_generation=False, return_loss=True, return_generation=False):
        ...
        if not is_generation:
            ...
            in_hidden_state = in_hidden_state[tte_obs_mask == 1]                                             # torch.Size([K=bsz * (seq_len-1) - MASK, hidden_size])
            assert ((k > 0) & (k <= self.vocab_size - 1)).all(), torch.unique(k, return_counts=True)

            logits = self.mlp(in_hidden_state)                                                               #  torch.Size([K, vocab_size])
        ...
        output_dict ={"k": [k],
                      "logits": [logits[:, i].detach().cpu() for i in range(logits.shape[1])]                                # [torch.Size([K,]) for _ in range(vocab_size)]      
                     }
...
def setup_t_mlp_experiment(cfg, dm, vocab_size, checkpoint=None, logger=None):
    ...
    if checkpoint is None:
        next_event_experiment = CausalTMLPExperiment(cfg=cfg,
                                                     vocab_size=vocab_size,
                                                     )
    else:
        next_event_experiment = CausalTMLPExperiment.load_from_checkpoint(checkpoint,
                                                                         cfg=cfg)
    ...
    return next_event_experiment, CausalTMLPExperiment, _trainer
